...
当前前端主入口已调整为按标的搜索预览：/api/instruments 提供可搜索下拉，/api/raw-instrument-preview 展示原始数据宽表，/api/factor-preview 展示处理后的日频因子宽表。
原始数据预览只展示数据宽表，不展示 raw 文件索引、记录数、缺失统计或底层 JSON。
