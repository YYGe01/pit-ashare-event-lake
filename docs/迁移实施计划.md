...
```text
新增 data_coverage 统计：每个 silver 数据集的行数、数据源、日期范围、覆盖标的数。
核心完整度按 daily_bar、adj_factor、price_limit 三个必备日频维度统计标的缺失情况。
news、announcement、trade_status、daily_news_factor、daily_announcement_factor 作为稀疏维度，只统计有记录覆盖。
```

数据预览页改造：

```text
dataset-preview 返回单表覆盖摘要、字段过滤能力、字段列表和样例行。
instrument-timeline 按标的合并 daily_bar、adj_factor、price_limit、trade_status、daily_news_factor、daily_announcement_factor，并返回 news/announcement 明细。
前端数据预览页提供“按数据集看”和“按标的看”两个只读视角；标的时间线按行情价格、复权/涨跌停/交易状态、新闻因子拆解、公告因子拆解分组展示。
当前前端主入口已调整为按标的搜索预览：/api/instruments 提供可搜索下拉，/api/raw-instrument-preview 展示 raw JSON 原始采集记录，/api/factor-preview 展示处理后的日频因子。
```
