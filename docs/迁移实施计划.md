...
按标的核心维度覆盖分数按 daily_bar、adj_factor、price_limit 三个必备日频维度统计，并在明细列里显示已覆盖/应覆盖，例如 484/484；不再展示独立缺口维度。
...
```text
数据预览页使用“覆盖情况 / 原始数据预览 / 处理后因子预览”三个 tab；覆盖情况 tab 合并展示源表覆盖和按标的核心维度覆盖分数。
...
原始数据预览一行一个日期，横向展示日线行情、复权、涨跌停、交易状态等采集维度；新闻和公告只显示数量，点击数量展开当天明细。
