...
qdc crawl-daily --date 2026-05-13 --source-id yicai --symbols "SH601991,SZ300965" --page-size 20 --max-pages 1
...
日线行情 `daily_bar`、复权因子 `adj_factor`、涨跌停价格 `price_limit` 可用 `--universe` 展开上游代码 `symbol`，也可以显式传入 `--symbols` 覆盖。历史回补命令仍保留 AkShare `announcement` / `news` 能力，但默认每日文档链路不再用 AkShare 历史公告新闻接口；少量厂商级快讯 opt-in 源会复用 AkShare 已封装的公开实时资讯函数。`qdc refresh-universe` 可把 AkShare 指数成分快照写入 `qdc_silver.universe_constituent`，回补规划会优先使用最新快照；如果没有快照，再回退到配置里的静态样例。

`qdc daily-pipeline` 是收盘后日频自动化入口，默认使用 `all_a` 全 A 当前 active 标的：先刷新 `stock_basic`，再执行结构化单日采集、同日新闻公告爬虫、因子重建、Parquet 同步、质量检查和 Qlib provider 导出。每日结构化任务只规划 `trade_calendar`、`daily_bar`、`adj_factor`、`price_limit` 和 `trade_status`；默认配置会按 `source_ids: [akshare, eastmoney, sina]` 规划结构化备源，其中 Eastmoney 覆盖日线、复权、涨跌停和停牌，Sina 覆盖交易日历。公告新闻只走 `crawl-daily` 的 crawler-backed 源，避免 AkShare 元数据混入当日文档口径。配置里的 `daily_parallelism`、`crawl_parallelism`、`crawl_request_timeout_seconds` 和 `crawl_source_timeout_seconds` 控制并行度和源级超时；命令行同名参数可临时覆盖。`crawl_documents: true` 会在因子构建前执行 `crawl-daily` 的默认新闻公告源；全市场 `all_a` 文档采集不再额外传几千只股票的 `instrument_filter`，显式 `--symbols` 或非全市场 universe 才会把标的列表作为本地过滤条件传给文档爬虫。Qlib 导出默认只从本次运行日期开始，避免 daily-only provider 混入旧数据；需要重导更早日期时再显式传 `--export-start <YYYY-MM-DD>`。需要临时只跑结构化链路时传 `--no-crawl-documents`。首次全市场运行前先用 `--symbols`、`--control-only` 和爬虫限量参数做 smoke。

如果暂时不使用历史回补数据，改用 `config/quant_data_center_daily_only.yaml`。该配置把 DuckDB、raw、Parquet、Qlib provider 和日志全部写到 `data/quant_data_center_daily_only/`，与当前 `data/quant_data_center/` 历史回补库隔离。`daily_pipeline` 段保存 `source_ids`、`batch_size`、`daily_parallelism`、`market_name`、非结构化文档爬虫和跳过步骤等默认参数；默认会采集公告和新闻文档，Qlib 导出起点默认等于本次 `daily-pipeline` 运行日期。命令行传同名参数时会覆盖配置，例如 `--batch-size 20`、`--source-ids akshare`、`--export-start 2026-05-01` 或 `--no-crawl-documents`。

`qdc crawl-plan` / `qdc crawl-run` / `qdc crawl-daily` 是非结构化数据每日爬虫入口。当前默认源是 `cninfo_announcement`、`sse_announcement`、`sina_finance_news`、`eastmoney_roll_news` 和 `nbd_company_news`。额外可按需用 `--source-id` 单独运行厂商级新闻源：`sina`、`wallstreetcn`、`10jqka`、`eastmoney`、`yuncaijing`、`fenghuang`、`jinrongjie`、`cls`、`yicai`。公告源必须能按请求日期或返回字段确认当日发布日期；新闻源必须能解析完整 `YYYY-MM-DD HH:MM[:SS]` 发布时间，分不清发布日期的候选不启用。Sina/Eastmoney 滚动页和各厂商快讯源更偏近实时口径，不适合作为隔天历史日期主源；NBD、金融界和云财经日期页可做有日期约束的 metadata 样本。文档源按 source 并行执行；单源请求或整体超时会标记该源本次失败，其他成功源继续落 raw 和 silver。raw 侧除原始 JSON 外，会额外写直观文件包 `data/quant_data_center/raw/documents/<date>/<source>/<run>/manifest.json` 和 `records.jsonl`；新闻 `records.jsonl` 只写已映射到 instrument 的 normalized 记录，整包上游响应只保留在 raw 对象中。DuckDB 继续做索引、控制表、silver 表、去重和导出。新闻源保存 metadata/title/url，并在来源列表或 API 已返回正文/摘要时保存最多 1200 字本地预览，不持久化完整文章 HTML。`build-factors` 会按 `publish_date + instrument + normalized_title` 对多源文档去重后再计数。可用 `--page-size` / `--max-pages` / `--pdf-limit` 做小范围真实 smoke；如只验证公告列表元数据，可加 `--skip-pdf-download`。
...
`qdc console` 会启动一个本地 Web 控制台，默认访问 `http://127.0.0.1:8765/`。当前控制台聚焦每日采集：今日总览先给“今天能不能用”的中文结论，再展示核心完整率、采集路线图、质量体检、供应商采集质量、数据集覆盖、批次明细和问题标的；英文术语会尽量附中文说明。今日总览里的“执行每日采集”按钮会在本地启动一次受限的 `qdc daily-pipeline --watch` 子进程，可设置标的、批次大小、是否刷新股票池、是否采集公告新闻和是否只跑控制面；同一控制台会限制一次只跑一个流水线。运行中会显示“停止采集”按钮，对当前子进程发出终止请求；如果中断后控制表残留 `running` 批次，需要先恢复陈旧 running 任务再重试。页面会 15 秒自动刷新，并把结构化采集批次和公告新闻爬虫批次逐条显示成进度条。数据预览支持按标的查看原始输入宽表和处理后日频因子宽表。采集过程中可以保持控制台打开；写入端遇到控制台瞬时只读锁会等待重试，控制台遇到采集写锁会显示“写入中”降级状态并继续刷新。
