...
docs/每日自动采集实施计划.md
docs/量化非结构化数据每日爬虫实施方案.md（归档，当前计划已合并到每日自动采集文档）
```
...
DuckDB 当前按单写进程使用；不要并行执行多个会写同一个 `qdc.duckdb` 的命令。`qdc console` 作为只读观察面可以和采集同时打开：写入端会对瞬时 DuckDB 锁做等待重试，控制台 API 遇到采集写锁时返回“写入中”降级状态并继续刷新。
...
已实现：轻量 `qdc console` 只读控制台已收敛到每日采集视图，展示今日总览、每日链路阶段进度、数据预览和质量信号；历史回补队列先不展示
...
```text
后端：src/quant_data_center/console.py，使用标准库 HTTP server 和 DuckDB read_only 连接；写锁期间返回 busy 降级 payload
...
DuckDB 仍应按单写进程运行；不要并行跑多个写入命令。`qdc console` 不再要求在采集前关闭，采集写入端会等待控制台瞬时只读锁，控制台在写锁期间显示 busy 降级状态。
```
...
2026-05-12 收敛每日采集、新闻公告爬虫和阶段状态到统一入口：
...
```text
daily-pipeline 已实现结构化每日采集、因子、同步、质量和 Qlib 导出；默认配置支持 `source_ids: [akshare, eastmoney, sina]` 结构化多源和 `daily_parallelism` 并行执行。
crawl-daily 已实现 cninfo_announcement、sse_announcement、sina_finance_news、eastmoney_roll_news 和 nbd_company_news 五个新闻公告爬虫源。
daily-pipeline --crawl-documents 已实现，可在因子构建前自动执行 crawl-daily 默认源。
daily-only 隔离配置已实现，当前只用每日新增数据时使用 config/quant_data_center_daily_only.yaml；daily_pipeline 段可保存默认参数，默认采集公告和新闻文档，命令行参数可覆盖配置。
每日公告和新闻口径只使用 crawler-backed 源：announcement=cninfo_announcement/sse_announcement，news=sina_finance_news/eastmoney_roll_news/nbd_company_news。AkShare announcement/news 仅作为历史回补遗留能力保留，不再进入 daily-pipeline、每日控制台明细或文本因子。
...
控制台公告/新闻标题优先预览本地保存内容：有 PDF 对象时嵌入本地 PDF；有 raw JSON / raw_records 时预览本地列表/元数据；没有本地对象时明确标记未保存内容。
文本因子按 publish_date + instrument + normalized_title 跨源去重，raw 和 silver 保留来源级证据。
并行抓取：文档源按 source 并行，支持 request/source timeout；结构化任务按 `daily_parallelism` 并行。DuckDB 写入使用短连接和锁等待重试，后续大规模运行如遇锁竞争，再继续拆分 fetch -> persist。
全市场真实每日运行、连续调度和 coverage 报告尚未验收。
历史回补先冻结，不作为当前默认计划、运行和控制台展示内容。
```
...
conda run -n ai-trader qdc daily-pipeline --date 2026-05-11 --symbols "SH600000,SZ000001" --batch-size 1 --control-only --crawl-documents --crawl-page-size 5 --crawl-max-pages 1 --crawl-pdf-limit 1
conda run -n ai-trader qdc --config config/quant_data_center_daily_only.yaml init
conda run -n ai-trader qdc --config config/quant_data_center_daily_only.yaml daily-pipeline
conda run -n ai-trader qdc crawl-daily --date 2026-05-11 --control-only
conda run -n ai-trader qdc crawl-daily --date 2026-05-11 --source-id cninfo_announcement --page-size 2 --max-pages 1 --pdf-limit 1
conda run -n ai-trader qdc crawl-daily --date 2026-05-11 --source-id sse_announcement --page-size 2 --max-pages 1 --skip-pdf-download
conda run -n ai-trader qdc crawl-daily --date 2026-05-11 --source-id sina_finance_news --page-size 10 --max-pages 1
conda run -n ai-trader qdc crawl-daily --date 2026-05-11 --source-id eastmoney_roll_news --page-size 10 --max-pages 1
```

本地当前真实进度：

```text
daily_bar / adj_factor / price_limit：CSI300 2023-01-03 -> 2024-12-31 已跑通，2026-05-11 两标的联动 smoke 已跑通，all_a 前 22 个 daily_bar 批次已跑通。
Qlib provider：2023-2024 CSI300 和 2026-05-11 两标的 smoke 均已导出并验证可读。
announcement：AkShare 2024 样本和 CNINFO 2026-05-11 小样本已入库并生成公告日频因子。
news：只有 2026-05-11 小样本补位。
crawler_source / crawl_task / crawl_run：已有一次 daily-pipeline --crawl-documents 小范围真实联动记录，尚无连续调度记录。
all_a 单日队列：2026-05-11 active 标的数 5515，batch-size 50，计划任务数 447；前 22 个 daily_bar 批次成功，当日 daily_bar 当前 994 行 / 994 标的、质量检查 0 issue；daily_bar 剩余 91 个 pending 批次。
口径修正：2026-05-11 SZ300001 当前可确认的 crawler-backed 当日公告为 1 条 CNINFO 记录；该记录本地保存了公告列表 raw JSON，PDF 因当时 `--crawl-pdf-limit 1` 被标记为 `skipped_by_limit`，没有正文文本抽取。先前弹窗看到的 13 条 AkShare 记录不再作为每日当日公告明细展示。
```

阶段状态：

```text
已完成：爬虫控制面、CNINFO/SSE 公告列表、CNINFO/SSE PDF 留档能力、Sina/Eastmoney 新闻 metadata-only 补位、raw/documents 直观文件包、daily-pipeline --crawl-documents、文本因子跨源去重、Qlib 字段导出和两标的真实联动 smoke。
部分完成：全市场任务规划、前 22 个 all_a daily_bar 批次和小样本真实采集。
未完成：all_a 全市场单日完整采集、连续 10 到 20 个交易日稳定性、PDF 正文抽取、公告类型标准化、SZSE/BSE 稳定补源、财联社/证券时报/中证网/上证报候选确认、授权历史新闻源；对历史 AkShare 文档因子需要按新口径重建或废弃旧因子表。
下一步：从每日 all_a 队列继续串行消费当日批次，观察耗时、失败率和上游限流；不再扩展历史回补。
```
