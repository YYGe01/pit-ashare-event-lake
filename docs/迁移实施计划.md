...
LLM 事件抽取接口已经预留在 `qdc classify-text-event --provider llm`，通过 `LITELLM_MODEL` 和运行时 API key 走 LiteLLM/DeepSeek 单条分类 smoke。该接口当前只用于验证后续切换契约，不参与公告、新闻的全量采集和 `qdc build-factors`。
