...
crawl-daily 已实现 cninfo_announcement 和 sina_finance_news 两个新闻公告爬虫源。
...
```text
已完成：爬虫控制面、CNINFO 公告列表和 PDF 留档、Sina 新闻 metadata-only 补位、daily-pipeline --crawl-documents、文本因子、Qlib 字段导出和两标的真实联动 smoke。
部分完成：全市场任务规划、前 22 个 all_a daily_bar 批次和小样本真实采集。
未完成：all_a 全市场单日完整采集、连续 10 到 20 个交易日稳定性、PDF 正文抽取、公告类型标准化、交易所公告补源、授权历史新闻源。
