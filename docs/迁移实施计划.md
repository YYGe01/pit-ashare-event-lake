...
按标的核心维度缺失数按 daily_bar、adj_factor、price_limit 三个必备日频维度统计，并在明细列里直接显示缺失交易日数；完整就是 0，不再展示独立缺口维度或分母说明。
news、announcement、trade_status 作为稀疏源表，只统计已有记录覆盖，不把没有事件判成缺失。
...
```text
数据预览页使用“覆盖情况 / 原始数据预览 / 处理后因子预览”三个 tab；覆盖情况 tab 合并展示源表覆盖和按标的核心维度缺失数。
