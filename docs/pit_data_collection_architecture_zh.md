...
正式写采集框架代码前，必须先确认账号/API Key、授权边界、是否需要手工下载历史数据、部署环境、存储路径、备份方式、告警通道和预算。完整清单见：
...
```text
第一阶段研究范围和 P0 数据集；
每个候选数据源是否允许采集、保存 raw、保存全文或附件；
需要登录/API Key/Token 的 source 对应 credential_ref；
是否有已购买或已下载的历史数据包需要导入；
data_lake 根目录、可用磁盘容量和备份位置；
运行机器是否长期在线；
失败告警通道；
付费源预算、额度和合同限制。
```

真实账号、密码、Token、Cookie 不进入 git、Markdown 或聊天记录，只通过 `.env`、系统环境变量、密码管理器或密钥管理服务提供。
