...
from pitlake.connectors.market.akshare_price_limit import AksharePriceLimitConnector
...
def test_akshare_price_limit_connector_infers_limits(
    ...
    contract_path = Path("config/dataset_contracts/price_limit.yaml")
    contract = DatasetContract.from_payload(load_yaml_file(contract_path), contract_path)
    connector = AksharePriceLimitConnector(
        settings=settings,
        source_config={
    ...
    run_id = metadata.create_run(
        ...
        provider_id="akshare",
        logical_dataset="price_limit",
        connector_name="AksharePriceLimitConnector",
    ...
    with metadata.connect() as conn:
        rows = conn.execute(
            ...
            where logical_dataset = 'price_limit'
            """
        ).fetchall()
        raw_count = conn.execute(
            "select count(*) as count from raw_object where source_id = 'ashare_price_limit'"
        ).fetchone()["count"]

    assert raw_count == 1
    assert len(rows) == 1
    assert rows[0]["source_item_key"] == "akshare:600001:2026-04-24"
    assert '"limit_up":11.0' in rows[0]["observed_payload_json"]
    assert '"limit_down":9.0' in rows[0]["observed_payload_json"]
    assert '"limit_rule":"main_board_normal_10pct_v0_inferred"' in rows[0]["observed_payload_json"]
