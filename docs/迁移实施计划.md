...
```text
已实现 crawler_source / crawl_task / crawl_run 控制表。
已实现 crawler source registry，首个 source_id 为 cninfo_announcement。
已实现 qdc crawl-plan / crawl-run / crawl-daily / crawl-list / crawl-recover-running。
已实现 --control-only 状态流。
已实现 cninfo_announcement 公告列表真实 fetcher：按日期分页请求、写 raw JSON、写 bronze Parquet、解析到 qdc_silver.announcement。
已实现 CNINFO 公开 PDF 原文下载：PDF 以 `raw_file` 层登记到 source_object，PDF hash、size、object_id 和下载状态回写到 qdc_silver.announcement。
已实现 sina_finance_news 公开新闻 metadata-only 每日补位：按滚动新闻分页获取标题、URL、发布时间，按 qdc_silver.stock_basic 的代码/名称映射 instrument，写 raw JSON、bronze Parquet 和 qdc_silver.news。
已实现 crawl-daily 默认同时规划 cninfo_announcement 和 sina_finance_news，新闻源产物复用现有 daily_news_factor 和 Qlib export 链路。
待实现：PDF 正文抽取、公告类型标准化、交易所公告补源覆盖差异报告，以及 20 个交易日连续运行后的付费历史源评估。
```
