...
```text
daily_bar / adj_factor / price_limit：CSI300 2023-01-03 -> 2024-12-31 已跑通。
Qlib provider：2023-2024 CSI300 已导出并验证可读。
announcement：AkShare 2024 样本已入库并生成公告日频因子。
news：只有 2026-05-11 小样本补位。
crawler_source / crawl_task / crawl_run：当前本地库没有连续调度记录。
```

阶段状态：

```text
已完成：爬虫控制面、CNINFO 公告列表和 PDF 留档、Sina 新闻 metadata-only 补位、daily-pipeline --crawl-documents、文本因子和 Qlib 字段导出。
部分完成：daily-pipeline 小范围 control-only、CNINFO/Sina 小范围真实 smoke。
未完成：all_a 全市场单日真实采集、连续 10 到 20 个交易日稳定性、PDF 正文抽取、公告类型标准化、交易所公告补源、授权历史新闻源。
下一步：先跑 daily-pipeline --crawl-documents 真实小范围完整链路，再推进 all_a 全市场单日运行。
```
