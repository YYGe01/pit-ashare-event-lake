...
已实现：轻量 `qdc console` 只读控制台已收敛到每日采集视图，今日总览先展示中文可用性结论、核心完整率、采集路线图、质量体检和供应商采集质量，再展示数据集覆盖、批次明细、问题标的和数据预览；历史回补队列先不展示
...
API：daily-collection-status / daily-wide-preview / instruments / source-object-file，以及保留的 overview / quality-issues / source-objects / dataset-preview
页面：今日总览、数据预览
...
```text
总览页保留今日结论、核心完整率、采集路线图、质量体检、供应商采集质量、数据集覆盖、批次明细和问题标的。
