...
```text
daily-pipeline 已实现结构化每日采集、因子、同步、质量和 Qlib 导出。
crawl-daily 已实现 cninfo_announcement、sse_announcement、sina_finance_news 和 eastmoney_roll_news 四个新闻公告爬虫源。
...
每日公告和新闻口径只使用 crawler-backed 源：announcement=cninfo_announcement/sse_announcement，news=sina_finance_news/eastmoney_roll_news。AkShare announcement/news 仅作为历史回补遗留能力保留，不再进入 daily-pipeline、每日控制台明细或文本因子。
...
并行抓取计划：不同网站的 fetch 可并行，但当前 adapter 会写 DuckDB，先保持串行单写；下一步拆成 fetch -> persist，fetch 阶段并行写 raw/documents，persist 阶段单进程去重写库。
