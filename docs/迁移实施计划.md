...
当前前端主入口已调整为按标的搜索预览：/api/instruments 提供可搜索下拉，/api/raw-instrument-preview 展示按日期聚合的原始数据宽表，/api/factor-preview 展示处理后的日频因子宽表。
原始数据预览一行一个日期，横向展示日线行情、复权、涨跌停、交易状态等全部原始源字段；新闻和公告只显示数量，点击数量展开当天明细。
处理后因子预览中的 news_count / announcement_count 默认只展示数量，点击数量展开当天新闻或公告详情。
