# A 股 Point-in-Time 日/周频数据采集层实施方案
...
本文默认服务的是日频/周频量化研究，而不是高频交易或分钟级择时。
...
因此，采集框架的默认优先级不是“越实时越好”，而是：

```text
盘后日线、复权、交易日历、公告、财报披露、监管政策、宏观/商品/全球市场日频指标优先；
分钟级行情快照、Level-2、tick、逐笔委托不进入第一阶段默认范围；
对公告和政策可以做低频轮询，目的是保护 PIT 可见时间，而不是做盘中交易。
```

如果后续只做每周调仓，大部分源每天采 1-4 次就足够；如果做每日盘后调仓，重点保证收盘后、晚间公告后、次日开盘前三个时间点的数据完整。
...
| 数据源类别 | 价值 | 采集频率 | 说明 |
| --- | --- | --- | --- |
| A 股盘后日线/复权/基础行情 | 日/周频模型的价格和成交量底座 | 16:30、20:00、次日 08:30 兜底 | 先做日线、复权因子和基础行情，不做分钟级默认采集 |
| 交易日历/停复牌/涨跌停 | 回测、标签和候选池约束 | 每日盘前 + 盘后 | 研究层最基础约束 |
| 上市公司公告/财报披露文件 | 公司级事件和基本面信息 | 交易日每 1-2 小时，盘后 20:00/23:00 兜底 | 巨潮、交易所；保存列表页、PDF 和附件 |
| 交易所/监管信息 | 规则和监管冲击 | 每日 2-4 次，盘后兜底 | 证监会、交易所 |
| 政策/部委新闻 | A 股政策驱动 | 每日 1-4 次 | 政府网、央行、发改委等 |
| 商品期货/大宗价格 | 周期行业外部变量 | 盘后结算价/收盘价 + 次日兜底 | 国内期货和国际商品，优先日频连续序列 |
| 全球市场指标 | 外部风险偏好 | 每日收盘后 + 次日早晨 | 美股、美元、美债、VIX、原油、黄金 |

### 6.2 P1：稳定后再接入

| 数据源类别 | 价值 | 难点 |
| --- | --- | --- |
| GDELT/全球新闻 | 地缘、灾害、国际事件 | 低成本但噪声高，先做日频/6 小时级汇总 |
...
| 数据源类别 | 价值 | 为什么后做 |
| --- | --- | --- |
| A 股分钟级/盘中快照 | 盘中状态和更细粒度回放 | 对日/周频调仓不是刚需，存储和稳定性成本更高 |
...
- 要保留原始 payload。
- 盘后日线缺失要记录，不要静默跳过。
- 复权因子、停复牌、涨跌停要和日线分开建 logical_dataset，避免字段口径混在一起。
- 如未来增加分钟级快照，应作为独立 P2 数据集，不影响日/周频主路径。
...
### 8.5 market_daily_raw

日/周频研究优先建立盘后日线观测表，但仍然必须保留 raw payload。
...
```text
优先保存盘后日线、复权因子、成交额、换手率、停复牌和涨跌停
保存完整 raw payload，不要只保存标准字段
日频数据的可复盘性和口径稳定性比分钟级新鲜度更重要
分钟级快照如未来接入，应作为独立 optional dataset
```
...
| 数据 | 频率 |
| --- | --- |
| A 股盘后日线/复权 | 16:30、20:00、次日 08:30 兜底 |
| 交易日历/停复牌/涨跌停 | 每日盘前 + 盘后 |
| 公告/财报披露文件 | 交易日每 1-2 小时，盘后 20:00/23:00 兜底 |
| 交易所/监管新闻 | 每日 2-4 次，盘后兜底 |
| 部委政策 | 每日 1-4 次 |
| GDELT/国际新闻 | 每 6 小时或每日汇总 |
| 财经 RSS | 每 1-2 小时，至少每日兜底 |
| 商品期货 | 盘后结算价/收盘价 + 次日兜底 |
| 全球市场 | 海外收盘后 + 次日早晨 |
| 天气灾害 | 每日；灾害期可提高到每 6 小时 |
| 宏观数据 | 按发布日历 + 每日兜底 |
| A 股分钟级/盘中快照 | P2 可选，不进入默认日/周频框架 |
...
5. 实现公告采集器
6. 实现政策/监管新闻采集器
7. 实现低频新闻/RSS 采集器，GDELT 可延后到 P1
8. 实现商品/全球市场日频指标采集器
