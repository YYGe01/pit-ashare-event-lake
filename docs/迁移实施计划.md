...
两者当前仍是独立命令；daily-pipeline 尚未自动调用 crawl-daily。
...
```text
已完成：爬虫控制面、CNINFO 公告列表和 PDF 留档、Sina 新闻 metadata-only 补位、文本因子和 Qlib 字段导出。
部分完成：daily-pipeline 小范围 control-only、CNINFO/Sina 小范围真实 smoke。
未完成：daily-pipeline --crawl-documents、all_a 全市场单日真实采集、连续 10 到 20 个交易日稳定性、PDF 正文抽取、公告类型标准化、交易所公告补源、授权历史新闻源。
下一步：先实现或明确调度 daily-pipeline 与 crawl-daily 的串行关系，再跑真实小范围完整链路。
```
