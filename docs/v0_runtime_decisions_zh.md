...
`pitlake console` / `pitlake ui` 当前提供本地只读 Web 控制台，默认监听 `127.0.0.1:8765`。它直接读取 `pitlake.sqlite`、`source_registry.yaml`、dataset contract、quality report、reconciliation report、manifest 和 raw 文件 metadata，用于查看每日采集健康、source x date 状态矩阵、dataset/source 状态、股票覆盖、运行批次、质量问题、只读 issue 队列、质量治理评分、volume baseline/schema drift、source health、对账问题、manifest 快照、raw 证据链、阶段状态、BI 连接提示和 JSON/CSV 导出 API。
...
控制台属于采集层观测入口，不在本仓库引入研究层、交易逻辑或可写运维操作。issue 状态当前只做派生只读 open 队列，不写入 acknowledged/resolved；外部 BI 只提供 DuckDB/Superset/Metabase 连接提示，不由控制台启动服务；PDF/HTML 仅按 raw 授权边界预览 metadata 或文本。第一版不依赖外部前端构建链，使用 Python 标准库 HTTP 服务和静态页面；后续如升级 FastAPI/React，API 语义仍应保持 `source registry -> run ledger -> quality/reconciliation -> raw evidence` 的证据链口径。
