"""AkShare connector for commodity futures daily bars."""
...
class AkshareCommodityDailyConnector(BaseConnector):
    """Collect selected domestic commodity futures daily bars."""
    ...
    def collect(self, *, run_id: str, options: dict[str, Any] | None = None) -> RunStats:
        ...
        for symbol in symbols:
            ...
            try:
                df = akshare.futures_zh_daily_sina(symbol=symbol)
                ...
                raw = self.raw_store.put_json(
                    ...
                    payload=raw_payload,
                    run_id=run_id,
                    filename_prefix=f"commodity_{symbol}_{target_date.replace('-', '')}",
                    metadata={"symbol": symbol, "akshare_function": "futures_zh_daily_sina"},
                )
                self.metadata_store.insert_raw_object(
                    raw,
                    request_hash=sha256_json(
                        {"function": "futures_zh_daily_sina", "symbol": symbol}
                    ),
                    request_url="akshare://futures_zh_daily_sina",
                ...
                self.metadata_store.insert_quality_results(
                    [
                        CheckResult(
                            check_name="akshare_futures_zh_daily_sina_request",
                            check_type="source_error",
                            severity="critical",
                            status="fail",
                            expected_value="successful AkShare commodity futures response",
        ...
        *,
        ...
        for record in df.to_dict(orient="records"):
            ...
            checks = quality.check_required_fields(
                contract=self.contract, payload=observed, run_id=run_id, source_id=self.source_id
            )
            ...
            self.metadata_store.insert_raw_item_version(
                ...
                source_id=self.source_id,
                source_item_key=observed["source_item_key"],
                title=f"{symbol} commodity daily {observed['trading_date']}",
                source_url="akshare://futures_zh_daily_sina",
    ...
    def _normalize_record(self, *, record: dict[str, Any], symbol: str, raw: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "settlement": _as_float(record.get("settle")),
            "volume": _as_float(record.get("volume")),
            "open_interest": _as_float(record.get("hold")),
            "session": "daily",
            "first_seen_at": raw.first_seen_at,
            "raw_uri": raw.raw_uri,
            "content_hash": raw.content_hash,
