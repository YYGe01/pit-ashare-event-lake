...
DuckDB 当前按单写进程使用；不要并行执行多个会写同一个 `qdc.duckdb` 的命令。`qdc console` 可以作为观察面和采集同时打开，也可以从今日总览启动一个受限的本地 `daily-pipeline --watch` 子进程；控制台会限制同一会话一次只跑一个流水线。写入端会对瞬时 DuckDB 锁做等待重试，控制台 API 遇到采集写锁时返回“写入中”降级状态并继续刷新。
...
已实现：轻量 `qdc console` 控制台已收敛到每日采集视图，今日总览先展示中文可用性结论、核心完整率、采集路线图、质量体检和供应商采集质量，再展示数据集覆盖、批次明细、问题标的和数据预览；支持本地启动一个受限 `daily-pipeline --watch` 子进程并逐条展示结构化批次和文档爬虫批次进度；历史回补队列先不展示
...
边界：页面只提供每日流水线启动入口，不接收任意 shell 命令；同一控制台会话只允许一个 `daily-pipeline` 子进程运行；历史回补队列不在控制台触发
```
