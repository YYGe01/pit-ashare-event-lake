...
daily-only 隔离配置已实现，当前只用每日新增数据时使用 config/quant_data_center_daily_only.yaml；daily_pipeline 段可保存默认参数，命令行参数可覆盖配置。
...
```text
daily_bar / adj_factor / price_limit：CSI300 2023-01-03 -> 2024-12-31 已跑通，2026-05-11 两标的联动 smoke 已跑通，all_a 前 6 个 daily_bar 批次已跑通。
...
all_a 单日队列：2026-05-11 active 标的数 5515，batch-size 50，计划任务数 447；前 6 个 daily_bar 批次成功写入 299 行，当日 daily_bar 当前 301 行、质量检查 0 issue。
```
...
部分完成：全市场任务规划、前 6 个 all_a daily_bar 批次和小样本真实采集。
未完成：all_a 全市场单日完整采集、连续 10 到 20 个交易日稳定性、PDF 正文抽取、公告类型标准化、交易所公告补源、授权历史新闻源。
下一步：从 pending 队列继续串行消费 all_a 批次，观察耗时、失败率和上游限流。
```
